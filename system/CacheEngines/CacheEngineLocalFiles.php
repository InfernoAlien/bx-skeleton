...
/**
 * Данный класс является практически полной копипастой Битриксового файлового движка для кэша.
 * Единственное отличие - в нём происходит изменение путей $baseDir с /bitrix/cache и /bitrix/managed_cache на
 * /local/cache и /local/managed_cache (метод replaceBitrixBaseDirToLocal())
 *
 * Делается это для того чтобы избежать коллизий кэша для множества площадок с одной базой в многостейджевой архитектуре.
 * Данный движок не предназначен для использования в боевом окружении.
 */
class CacheEngineLocalFiles
    implements ICacheEngine, ICacheEngineStat
{
     ...
     * @return string
     */
    public function replaceBitrixBaseDirToLocal($baseDir)
    {
        $search = ['/bitrix/managed_cache', '/bitrix/cache'];
        $replace = ['/local/managed_cache', '/local/cache'];

        return str_replace($search, $replace, (string) $baseDir);
    }
    ...
    {
        $baseDir = $this->replaceBitrixBaseDirToLocal($baseDir);
        $documentRoot = Main\Loader::getDocumentRoot();
    ...
    public function write($arAllVars, $baseDir, $initDir, $filename, $TTL)
    {
        ...
        if ($handle = fopen($fnTmp, "wb+"))
        {
            ...
            // дополнительно исправим права на файл.
